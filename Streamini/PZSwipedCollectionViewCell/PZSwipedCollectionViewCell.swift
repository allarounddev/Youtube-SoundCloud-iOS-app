...
class PZSwipedCollectionViewCell: UICollectionViewCell,UIGestureRecognizerDelegate {
    ...
    func successStreams(_ streams: [Stream]) {
        ...
        let index = self.superView(of: UICollectionView.self)?.indexPath(for: self)?.row
        let stream = self.streams[index!]
        
        StreamConnector().delete(stream.id, success: closeStreamSuccess, failure: closeStreamFailure)
    ...
    func closeStreamSuccess() {
        NotificationCenter.default.post(Notification(name: Notification.Name(rawValue: "refreshData"), object: nil))
        NotificationCenter.default.post(Notification(name: Notification.Name(rawValue: "refreshEpisodesData"), object: nil))
