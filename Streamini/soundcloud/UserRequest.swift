...
public extension SoundCloudUser {
     ...
     */
    @discardableResult
    public static func user(identifier: Int, completion: @escaping (SimpleAPIResponse<SoundCloudUser>) -> Void) -> CancelableOperation? {
        ...
        let request = Request(url: url, method: .get, parameters: parameters, parse: {
            if let user = SoundCloudUser(JSON: $0) {
                return .success(user)
            }
     ...
     */
    @discardableResult
    public static func search(query: String, completion: @escaping (PaginatedAPIResponse<SoundCloudUser>) -> Void) -> CancelableOperation? {
        guard let clientIdentifier = SoundcloudClient.clientIdentifier else {
            completion(PaginatedAPIResponse(error: .credentialsNotSet))
            return nil
        }

        let parse = { (JSON: JSONObject) -> Result<[SoundCloudUser], SoundcloudError> in
            guard let users = JSON.flatMap(transform: { SoundCloudUser(JSON: $0) }) else {
                return .failure(.parsing)
            }
        ...
        let request = Request(url: BaseURL, method: .get, parameters: parameters, parse: { JSON -> Result<PaginatedAPIResponse<SoundCloudUser>, SoundcloudError> in
    ...
    public func tracks(completion: @escaping (PaginatedAPIResponse<Track>) -> Void) -> CancelableOperation? {
        return SoundCloudUser.tracks(from: identifier, completion: completion)
    }
...
//            }
    ...
    public func comments(completion: @escaping (PaginatedAPIResponse<Comment>) -> Void) -> CancelableOperation? {
        return SoundCloudUser.comments(from: identifier, completion: completion)
    }
    ...
    public func favorites(completion: @escaping (PaginatedAPIResponse<Track>) -> Void) -> CancelableOperation? {
        return SoundCloudUser.favorites(from: identifier, completion: completion)
    }
     ...
     */
    @discardableResult
    public func followers(completion: @escaping (PaginatedAPIResponse<SoundCloudUser>) -> Void) -> CancelableOperation? {
        return SoundCloudUser.followers(from: identifier, completion: completion)
    }
     ...
     */
    @discardableResult
    public static func followers(from userIdentifier: Int, completion: @escaping (PaginatedAPIResponse<SoundCloudUser>) -> Void) -> CancelableOperation? {
        guard let clientIdentifier = SoundcloudClient.clientIdentifier else {
            completion(PaginatedAPIResponse(error: .credentialsNotSet))
            return nil
        }

        let url = SoundCloudUser.BaseURL.appendingPathComponent("\(userIdentifier)/followers.json")
        let parameters = ["client_id": clientIdentifier, "linked_partitioning": "true"]
        ...
        let request = Request(url: url, method: .get, parameters: parameters, parse: { JSON -> Result<PaginatedAPIResponse<SoundCloudUser>, SoundcloudError> in
            return .success(PaginatedAPIResponse(JSON: JSON, parse: parse))
        }) { result in
            completion(result.recover { PaginatedAPIResponse(error: $0) })
        }
     ...
     */
    @discardableResult
    public func followings(completion: @escaping (PaginatedAPIResponse<SoundCloudUser>) -> Void) -> CancelableOperation? {
        return SoundCloudUser.followings(from: identifier, completion: completion)
    }
     ...
     */
    @discardableResult
    public static func followings(from userIdentifier: Int, completion: @escaping (PaginatedAPIResponse<SoundCloudUser>) -> Void) -> CancelableOperation? {
        guard let clientIdentifier = SoundcloudClient.clientIdentifier else {
            completion(PaginatedAPIResponse(error: .credentialsNotSet))
            return nil
        }

        let url = SoundCloudUser.BaseURL.appendingPathComponent("\(userIdentifier)/followings.json")
        let parameters = ["client_id": clientIdentifier, "linked_partitioning": "true"]
    ...
    public func follow(userIdentifier: Int, completion: @escaping (SimpleAPIResponse<Bool>) -> Void) -> CancelableOperation? {
        #if !os(tvOS)
            return SoundCloudUser.changeFollowStatus(follow: true, userIdentifier: userIdentifier, completion: completion)
        #else
            return nil
        #endif
    }
    ...
    public func unfollow(userIdentifier: Int, completion: @escaping (SimpleAPIResponse<Bool>) -> Void) -> CancelableOperation? {
        #if !os(tvOS)
            return SoundCloudUser.changeFollowStatus(follow: false, userIdentifier: userIdentifier, completion: completion)
        #else
            return nil
        #endif
    }
...
//        #endif
    ...
    public func playlists(completion: @escaping (PaginatedAPIResponse<Playlist>) -> Void) -> CancelableOperation? {
        return SoundCloudUser.playlists(from: identifier, completion: completion)
    }
