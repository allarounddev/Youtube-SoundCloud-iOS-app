...
class JSONObject {
    ...
    subscript(key: String) -> JSONObject {
        ...
        if dict?["track"] != nil{
            return (dict?["track"]).map { JSONObject(($0 as AnyObject).object(forKey: key)) } ?? JSONObject(nil)
        }
        else{
            return (value).map { JSONObject(($0 as AnyObject).object(forKey: key)) } ?? JSONObject(nil)
        }
...
extension JSONObject {
    ...
    var urlStreamValue: URL? {
        return (value as? String).map { URL(string: "\($0)/stream?client_id=\(SoundcloudClient.clientIdentifier!)")}!//appendingQueryString("/stream?client_id=\(SoundcloudClient.clientIdentifier!)") } ?? nil
    }
